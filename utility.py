...
def validate_initial_parameters():
    ...
    random_key_addition_regex_check = check_regex_match(str(random_key_vector_addition_range),
                                                        constants.regex_pattern_digit)
...
def convert_text_to_bit(text):
    res = ''.join(format(i, '08b') for i in bytearray(text, encoding='utf-8'))
    return res


def convert_bit_to_text(bits, bit_length):
    """
